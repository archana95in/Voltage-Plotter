...
QJSlowFIRFilter::QJSlowFIRFilter(QObject *parent) : QObject(parent)
{
    Cof.push_back(1.0);
    reset();
}

void QJSlowFIRFilter::setKernel(QVector<kffsamp_t> imp_responce)
{
    Cof=imp_responce;
    reset();
}

void QJSlowFIRFilter::reset()
{
    Buf.fill(0,Cof.size());
    BufPtr=0;
}
